...
page = url %>% read_html() # takes the URL and parses the HTML content. 

page # if you look at the page object you observe that it has HTML inside of it so now we can do things like extract elements, tables..texts...etc. 
...
stats = page %>%  html_element(css = '#stats') %>%   
         html_table()
stats
...
stats_playoffs <- page %>%  html_element(css = '#stats_playoffs') %>%   
         html_table()
...
# check fo the duplicate columns 
...
GS_index <- grep('^GS$',  colnames(stats)) # should be 10
GS_index <- GS_index + 1
...
### All coded above in one single Single and put into a Function.
...
return(stats)  # return the data set itself 
}
```
...
all_column_names <- c(colnames(stats_regular), colnames(stats_playoffs)) %>% unique()
all_column_names
...
`%notin%` <- Negate(`%in%`) # the negate function nullifies whatever you pass in the parenthesis. 
...
```{r}
all_column_names[28] # you won't find this column in stats_playoffs 
```
...
missing_cols <- all_column_names[not_in_headers]
missing_cols
...
# it's going to go into the stats_playoffs data set and create columns and its going to initate those values to zero. 
...
write.csv(stats_combined, 'Scraped Tom Brady Stats.csv', na = '', row.names = F)
